...
use miden_faucet_backend::{ApiKey, Faucet, PoWConfig, Server as BackendServer};
use miden_faucet_common::{AssetOptions, network::FaucetNetwork, rpc_client::RpcClient};
use miden_faucet_frontend::Server as FrontendServer;
use miden_lib::{AuthScheme, account::faucets::create_basic_fungible_faucet};
use miden_node_utils::{
    crypto::get_rpo_random_coin, grpc::UrlExt, logging::OpenTelemetry, version::LongVersion,
};
use miden_objects::{
    Felt,
...
async fn run_faucet_command(cli: Cli) -> anyhow::Result<()> {
    match cli.command {
        ...
        Command::Start {
            ...
            let backend_router = BackendServer::new(
                asset_options.clone(),
                tx_requests,
            ...
            .router();
            let frontend_router = FrontendServer::new(faucet.faucet_id(), asset_options).router();
            ...
            let server_future = async {
                let server_handle = tokio::spawn(async move {
                    // TODO: eventually, we want to serve the backend and frontend separately in
                    // different addresses.
                    start_server(backend_router.merge(frontend_router), endpoint).await
                });
...
/// Serves the router by binding to the given URL.
async fn start_server(router: Router, url: Url) -> anyhow::Result<()> {
    ...
    info!(target: COMPONENT, address = %url, "Server started");
...
mod tests {
    ...
    use miden_faucet_common::stub_rpc_api::serve_stub;
